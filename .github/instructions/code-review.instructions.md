...
- **Tech Stack**: Python 3.12 (FastAPI, Celery, SQLAlchemy), Next.js 16 (TypeScript, Tailwind CSS), PostgreSQL 16, Redis, Azure Container Apps
- **Architecture**: Backend API + async Celery workers + Next.js frontend. Backend processes YouTube DJ sets via yt-dlp → ACRCloud audio fingerprinting → track identification.
- **Build Tool**: pip (backend), npm (frontend), Docker, Azure Bicep (infra)
- **Testing**: pytest (backend), Jest (frontend)
- **Code Style**: ruff (Python linting/formatting), ESLint + Prettier (TypeScript)
- **Authentication**: Azure Managed Identity for all cloud services (PostgreSQL, Redis, Blob Storage, Key Vault). No passwords in config. ACRCloud keys stored in Key Vault.
- **Deployment**: Azure Container Apps with Bicep templates, deploy.sh script. CI via GitHub Actions.
- **Conventions**: Conventional commits, feature branches (fix/*, feat/*), PRs required for main.
